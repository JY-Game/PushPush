...
//    };
    ...
    private void LevelIO()
    {
        ...
        //int levelCount = LevelIOTest(textAsset);
        //levels = new string[levelCount][]; //new string[Int32.Parse(lines[0])][];
        //levels[0] = new string[lines.Length];
        ...
        for (int i =0, y = 0, x = 0; i < lines.Length; i++)
        {
            ...
            if (line.StartsWith(";"))
            {
                //Debug.Log("New level added:"+line+"/"+lines[i-1][5]+"/"+Int32.Parse(lines[0]));
                //Debug.Log("New level added:" +"또는"+(int)'\r');//(int)lines[i-1][5]);
                //levels[y] = new string[x];
                y++;
               // levels[y] = new string[lines[i+1].Replace("\r", "").Length];
               x = 0;
                continue;
            }
